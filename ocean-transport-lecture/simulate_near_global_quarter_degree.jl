import Pkg
Pkg.activate("/home/ssilvest/experimental_oceananigans/Oceananigans.jl/")
using Statistics
using JLD2
using Printf
using Plots
...
using Oceananigans.ImmersedBoundaries: ImmersedBoundaryGrid, GridFittedBottom, peripheral_node, inactive_node
...
@inline is_immersed_drag_u(i, j, k, grid) = Int(inactive_node(Face(), Center(), Center(), i, j, k-1, grid) & !peripheral_node(Face(), Center(), Center(), i, j, k, grid))
@inline is_immersed_drag_v(i, j, k, grid) = Int(inactive_node(Center(), Face(), Center(), i, j, k-1, grid) & !peripheral_node(Center(), Face(), Center(), i, j, k, grid))                                
...
simulation.output_writers[:surface_fields] = JLD2OutputWriter(model, (; u, v, w, T, S, η),
                                                              schedule = TimeInterval(save_interval),
                                                              filename = output_prefix * "_200m",
                                                              indices = (:, :, 32), 
